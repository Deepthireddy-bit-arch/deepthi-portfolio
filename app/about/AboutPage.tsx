...
export default function AboutPage({ theme = 'white', data: override }: Props) {
  ...
  // Ticker pause/resume:
  // Desktop - CSS :hover handles it via animation-play-state in the stylesheet.
  // Mobile  - No hover exists. Tap-to-toggle: first tap pauses, second tap resumes.
  //           touchcancel (e.g. incoming call) always resumes.
  ...
  return (
    <main className={`${s.page} ${s.root}`}>
      ...
      {/* ══════════════════════════════════════════════════════════════
          HERO
          FIX: height is now controlled entirely by the CSS class.
          On desktop → 100vh. On mobile → 65vh / 58vh (see CSS).
          The inline paddingTop is removed — navbar offset is handled
          by the bottom padding on heroContent instead.
      ═════════════════════════════════════════════════════════════════ */}
      <section className={s.hero}>
        {/* Full-bleed background image */}
        <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
        ...
        {/* Text — always at bottom via section's align-items: flex-end */}
        <div className={s.heroContent}>
          <p className={s.heroHandle} data-hero-text>Frontend Developer · 1 Year In</p>
          <h1 className={s.heroHeading} data-hero-text>
            Dig&nbsp;<span className={s.heroHeadingItalic}>Deeper.</span>
